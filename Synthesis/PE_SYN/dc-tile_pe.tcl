...
#   report_power -hier
#   set_max_delay
#   write -format db -output tile_pe.db
#
# Doesn't work quite the way I expect
#   NameDesign = tile_pe    Set variable ok, but how to concatenate?
#   write_rtl -format verilog -output tile_pe.vg


#===== Set: make sure you change design name elsewhere in this file
set NameDesign "tile_pe"
...
set input_setup     [expr {$clk_period} + {$input_setup} ]
...
#===== Reports
write -format verilog -output tile_pe.vg -hierarchy $NameDesign
write_sdc	tile_pe.sdc
write_sdf	tile_pe.sdf


report_area               > tile_pe.area
report_cell               > tile_pe.cell
report_hierarchy          > tile_pe.hier
report_net                > tile_pe.net
report_power              > tile_pe.pow
report_timing -nworst 10  > tile_pe.tim
